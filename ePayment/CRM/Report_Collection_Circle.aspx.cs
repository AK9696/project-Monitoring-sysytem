...
public partial class Report_Collection_Circle : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //    }
            //}
            get_Circle_Wise_Details(Scheme_Id, Zone_Id);
        }
    }

    private void get_Circle_Wise_Details(int Scheme_Id, int Zone_Id)
    {
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_Circle_Wise_Details(Scheme_Id, Zone_Id);
        if (AllClasses.CheckDataSet(ds))
        {
    ...
    protected void lnkDistrict_Click(object sender, EventArgs e)
    {
        ...
        string District_Id = (lnkDistrict.Parent.Parent as GridViewRow).Cells[0].Text;
        string District_Name = lnkDistrict.Text.Trim();
        int Scheme_Id = 0;
        if (Request.QueryString.Count > 0)
        {
            Scheme_Id = Convert.ToInt32(Request.QueryString[0].ToString());
        ...
        Response.Redirect("Report_Collection_ULB.aspx?Scheme_Id=" + Scheme_Id.ToString() + "&District_Id=" + District_Id + "&District_Name=" + District_Name);
    }
}