...
public partial class MasterCoverLetter : System.Web.UI.Page
{
    ...
    protected void btnEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        get_tbl_PackageInvoice(Convert.ToInt32(hf_ProjectWork_Id.Value));
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (Session["UserType"].ToString() == "1")
        {
            ds = (new DataLayer()).get_tbl_ProjectWorkPkg_Invoice(0, Project_Id, 0, Zone_Id, Circle_Id, Division_Id, "", "", 0, 0);
        }
        else
        {
            ds = (new DataLayer()).get_tbl_ProjectWorkPkg_Invoice(0, Project_Id, 0, Zone_Id, Circle_Id, Division_Id, "", "", Convert.ToInt32(Session["Person_BranchOffice_Id"].ToString()), Convert.ToInt32(Session["PersonJuridiction_DesignationId"].ToString()));
        }
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
    ...
    protected void btnOpenInvoice_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (ProcessedBy == "")
        {
            MessageBox.Show("Unable To Approve Invoice Please Finalize First.");
            return;
        }
        else
        {
            Response.Redirect("MasterGenerateCoverLetter.aspx?Invoice_Id=" + Invoice_Id);
        }
    }
